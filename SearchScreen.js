...
import { searchBooks, getCategoryLabels, LANGUAGES } from '../database/db';
...
export default function SearchScreen() {
  ...
  const runSearch = useCallback(async () => {
    setLoading(true);
    try {
      const rows = await searchBooks({ query, categoryLabel: categoryCode, categoryType, status });
      setResults(rows);
    } finally {
      setLoading(false);
    }
  ...
  useFocusEffect(useCallback(() => {
    getCategoryLabels().then(setCategories).catch(() => setCategories([]));
  }, []));
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.container}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterRow}>
          ...
          {categories.map((c) => (
            <FilterButton key={c} label={c} active={categoryCode === c} onPress={() => setCategoryCode(c)} />
          ))}
        </ScrollView>
          ...
          renderItem={({ item }) => (
            <View style={styles.resultCard}>
              ...
              <View style={styles.tagsRow}>
                <Tag text={item.category_label || `${item.category_code}-${item.category_no}`} />
