// database/db.js
// Shared cloud database layer using Firebase Firestore — every phone with
// this app reads and writes the SAME data, so all Brothers Vayanasala
// members see each other's changes.
//
// Every function here keeps the EXACT same name/signature as the old
// on-device version, so none of the screens needed to change.
import {
  collection,
  doc,
  getDoc,
...
const issuesCol = collection(firestore, 'issues');
const countersRef = doc(firestore, 'meta', 'counters');

// ---------- Setup ----------

export async function initDatabase() {
  // Make sure the shared counters document exists (used for auto-numbering).
  const snap = await getDoc(countersRef);
  if (!snap.exists()) {
    await setDoc(countersRef, { nextBookId: 1, categoryCounters: {} });
  }
  return true;
}
...
export async function addBook({
  ...
  await setDoc(doc(booksCol, String(bookId)), {
    book_id: bookId,
...
export async function addBookExplicit({
  ...
  const existing = await getDoc(doc(booksCol, String(bookId)));
...
export async function searchBooks({ query: q, categoryCode, status }) {
  // Firestore doesn't support SQL-style LIKE/OR text search across fields,
  // so we fetch and filter client-side — fine for a library-sized collection.
...
export async function addMember({ memberId, name, phone, address }) {
  await setDoc(doc(membersCol, memberId), {
    member_id: memberId,
    name: name || null,
    phone: phone || null,
...
export async function upsertMember(memberId, name = '', phone = '', address = '') {
  const existing = await getDoc(doc(membersCol, memberId));
  await setDoc(
...
export async function getMember(memberId) {
  const snap = await getDoc(doc(membersCol, memberId));
...
export async function issueBook({ memberId, bookId }) {
  ...
  await setDoc(doc(booksCol, String(book.book_id)), { status: 'issued' }, { merge: true });
...
export async function returnBook({ bookId }) {
  ...
  if (activeIssue) {
    await setDoc(
      doc(issuesCol, activeIssue.id),
      { return_date: returnDateStr, status: 'returned' },
      { merge: true }
    );
  }
  await setDoc(doc(booksCol, String(book.book_id)), { status: 'available' }, { merge: true });

  return { returnDate: returnDateStr, book };
}

export async function getActiveIssues() {
  const [issuesSnap, books] = await Promise.all([getDocs(issuesCol), getAllBooks()]);
...
export async function restoreFullBackup({ books = [], members = [], issues = [] }) {
  ...
  for (const b of books) {
    await setDoc(doc(booksCol, String(b.book_id)), {
      book_id: b.book_id,
  ...
  for (const m of members) {
    await setDoc(doc(membersCol, m.member_id), {
  ...
  await setDoc(countersRef, { nextBookId: maxBookId + 1, categoryCounters });
