...
export default function AddBookScreen() {
  ...
  const handleImportExcel = async () => {
    try {
      ...
      const { inserted, failed } = await bulkAddBooks(rows);
...
function normalizeExcelRow(row) {
  ...
  const rawCategory = String(get(['categorycode', 'category'])).trim().toUpperCase();
  const validCodes = CATEGORIES.map((c) => c.code);
  const categoryCode = validCodes.includes(rawCategory) ? rawCategory : 'OT';
