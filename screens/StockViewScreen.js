...
import { getAllBooks, CATEGORIES } from '../database/db';
...
export default function StockViewScreen() {
  ...
  const load = async () => setBooks(await getAllBooks());
  ...
  const filteredBooks = books.filter((b) => {
    ...
    const matchesCategory = !category || b.category_code === category;
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.container}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filters}>
          <Filter label="All" active={!category} onPress={() => setCategory('')} />
          {CATEGORIES.map((c) => <Filter key={c.code} label={c.code} active={category === c.code} onPress={() => setCategory(c.code)} />)}
          ...
          renderItem={({ item }) => (
            <View style={styles.bookCard}>
              ...
              <View style={styles.detailsRow}>
                <Text style={styles.detail}>Category: <Text style={styles.detailStrong}>{item.category_code}-{item.category_no}</Text></Text>
