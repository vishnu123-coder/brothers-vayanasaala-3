...
export default function SearchScreen() {
  ...
  const runSearch = async (q, cat) => {
    const res = await searchBooks({ query: q, categoryCode: cat || null });
  ...
  const loadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const res = await searchBooks({ query, categoryCode: categoryCode || null, cursor });
      ...
      <ScreenHeader
        ...
        subtitle={
          cappedSearch
            ...
            : `${results.length} result(s)${hasMore ? ' — scroll for more' : ''}`
        }
        icon="search"
      />
