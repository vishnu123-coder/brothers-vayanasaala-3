...
export default function AddBookScreen() {
  ...
  const handleImportExcel = async () => {
    try {
      ...
      const rows = json.map((r) => normalizeExcelRow(r)).filter((r) => r.bookName);

      if (rows.length === 0) {
        Alert.alert('No rows found', 'Check that your file has a bookName column with data.');
        setImporting(false);
        return;
      }

      const { inserted, updated, failed } = await bulkAddBooks(rows);
  ...
  return (
    ...
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 60 }}>
      ...
      <FieldLabel text="Category ID / Number" />
      <TextInput
        style={styles.input}
        placeholder="e.g. SD77 (leave blank for automatic category)"
        value={form.categoryLabel}
        onChangeText={(v) => update('categoryLabel', v)}
        autoCapitalize="characters"
      />

      <FieldLabel text="Library Category" />
      <View style={styles.pickerWrap}>
        <Picker selectedValue={form.categoryCode} onValueChange={handleCategoryChange}>
          {CATEGORIES.map((c) => (
      ...
      <Text style={styles.helperText}>
        ...
        publicationName, cost, barcode (barcode optional). Book ID is generated automatically. The category value from the spreadsheet is preserved exactly.
      </Text>
...
function normalizeExcelRow(row) {
  // Accepts flexible header casing/spacing from the spreadsheet.
  const get = (keys) => {
  ...
  const rawCategory = String(get(['category', 'categorycode', 'categoryid', 'categoryno', 'categorynumber'])).trim().toUpperCase();
  const validCodes = CATEGORIES.map((c) => c.code);
  const categoryCode = validCodes.includes(rawCategory) ? rawCategory : (rawCategory.match(/^[A-Z]+/)?.[0] || 'OT');

  return {
    bookId: parseInt(get(['bookid', 'id', 'stocknumber', 'booknumber']), 10) || null,
    categoryCode,
    categoryLabel: rawCategory,
    categoryType: String(get(['categorytype', 'language'])) || LANGUAGES[0],
    bookName: String(get(['bookname', 'name', 'title'])).trim(),
    authorName: String(get(['authorname', 'author'])).trim(),
    publicationName: String(get(['publicationname', 'publication', 'publisher'])).trim(),
    cost: parseFloat(get(['cost', 'price'])) || null,
...
const styles = StyleSheet.create({
  ...
  autoLabel: { color: '#C9D6E5', fontSize: 10 },
  autoValue: { color: '#fff', fontSize: 18, fontWeight: '800', marginTop: 4 },
