...
import { addMember, searchMembers, bulkImportMembers, deleteMember } from '../database/db';
...
export default function MembersScreen() {
  ...
  const handleDeleteMember = (item) => {
    Alert.alert(
      'Delete this member?',
      `${item.name || item.member_id} will be permanently removed. This can't be undone.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await deleteMember(item.member_id);
              load(query);
            } catch (e) {
